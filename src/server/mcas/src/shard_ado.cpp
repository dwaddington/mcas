...
void Shard::process_put_ado_request(Connection_handler* handler, const protocol::Message_put_ado_request* msg)
{
  ...
  if ((msg->flags & IMCAS::ADO_FLAG_DETACHED) && (msg->root_val_len > 0)) {
    ...
    size_t alignment = 0;
    status_t s = _i_kvstore->lock(msg->pool_id(), std::string(msg->key()), locktype, value, value_len, alignment, key_handle, &key_ptr);
    if (s < S_OK) {
  ...
  */
  if (!value) { /* now take the lock if not already locked */
    size_t alignment = 0;
    if (_i_kvstore->lock(msg->pool_id(), std::string(msg->key()), locktype, value, value_len, alignment, key_handle, &key_ptr) != S_OK) {
