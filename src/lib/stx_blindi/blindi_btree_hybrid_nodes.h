/**
 * \file include/stx/blindi_btree_hybrid_nodes.h
 * Contains the main B+ tree implementation template class blindi_btree_hybrid_nodes.
 */
...
#ifndef _STX_BLINDI_BTREE_HYBRID_NODES_H_
#define _STX_BLINDI_BTREE_HYBRID_NODES_H_
...
/// tree.
#define BTREE_FRIENDS           friend class blindi_btree_hybrid_nodes_friend;
#endif
...
struct blindi_btree_hybrid_nodes_default_set_traits
{
    ...
    /// As of stx-blindi_btree_hybrid_nodes-0.9, the code does linear search in find_lower() and
 ...
 * inner node sizes by assuming a cache line size of 256 bytes. */
template <int _InnerSlots, int _LeafSlots>
struct blindi_btree_hybrid_nodes_default_map_traits
{
    /// If true, the tree will self verify it's invariants after each insert()
    /// or erase(). The header must have been compiled with BTREE_DEBUG defined.
    static const bool   selfverify = false;
 ...
 *
 * This class is specialized into blindi_btree_hybrid_nodes_set, btree_multiset, btree_map and
 * blindi_btree_hybrid_nodes_multimap using default template parameters and facade functions.
 */
template <typename _Key, typename _Data, template<typename, int> typename BlindiBtreeHybridType,
	  int _InnerSlots, int _LeafSlots, 
          typename _Value = std::pair<_Key, _Data>,
          typename _Compare = std::less<_Key>,
          typename _Traits = blindi_btree_hybrid_nodes_default_map_traits<_InnerSlots, _LeafSlots>,
...
private:
    ...
    /// Eighth template parameter: boolean indicator whether the blindi_btree_hybrid_nodes is used
    ...
    typedef GenericBlindiSeqTreeNode<BlindiBtreeHybridType<key_type, traits::leafslots>, key_type, traits::leafslots> blindi_node1;
    typedef GenericBlindiSeqTreeNode<BlindiBtreeHybridType<key_type, traits::leafslots*2>, key_type, traits::leafslots*2> blindi_node2;
    typedef GenericBlindiSeqTreeNode<BlindiBtreeHybridType<key_type, traits::leafslots*4>, key_type, traits::leafslots*4> blindi_node4;
    typedef GenericBlindiSeqTreeNode<BlindiBtreeHybridType<key_type, traits::leafslots*8>, key_type, traits::leafslots*8> blindi_node8;
...
public:
    ...
    typedef blindi_btree_hybrid_nodes<key_type, data_type, BlindiBtreeHybridType,  _InnerSlots, _LeafSlots, value_type, key_compare,
                  traits, allow_duplicates, allocator_type, used_as_set> blindi_btree_hybrid_nodes_self;
...
public:
    ...
    struct node
    {
        ...
        uint8_t blindileaftype;
#ifdef BREATHING_SIZE 
        unsigned short  currentmaxslot;
#endif

#ifdef BREATHING_SIZE 
        /// Delayed initialisation of constructed node
        inline void initialize(const unsigned short l, const unsigned short cms)
        {
            level = l;
            slotuse = 0;
	    currentmaxslot = minleafslots + BREATHING_SIZE >  leafslotmax ? leafslotmax : minleafslots + BREATHING_SIZE;
        }
#endif
        /// Delayed initialisation of constructed node
        inline void initialize(const unsigned short l, uint8_t bln)
        {
            level = l;
            slotuse = 0;
            blindileaftype = bln;
	...
	inline bool isalreadycomprerss() const 
	{
		return alreadycomprerss;
	}
    ...
    struct leaf_node : public node
    {
       ...
       /// Set variables to initial values
        inline void initialize(uint8_t bln)
        {
            node::initialize(0, bln);
            prevleaf = nextleaf = NULL;
        }

        /// True if the node's slots are full
        inline bool isfullbtree() const
        {
        ...
        /// True if blindinodeleaf
        inline bool isblindileafnode() const
        {
            return node::blindileaftype > 0;
        }

        /// True if blindinodeleaf
        inline bool isblindi_can_extend() const
        {
#if (BLINDI_MAX_MULTI_SIZE == 1)
            return 0;
#endif
#if (BLINDI_MAX_MULTI_SIZE == 2)
	    if(node::blindileaftype == 2)
	    {
		return 0;		
	    }
            return 1;
#endif
#if (BLINDI_MAX_MULTI_SIZE == 4)
	    if(node::blindileaftype == 4)
	    {
		return 0;		
	    }
            return 1;
...
#endif
#if (BLINDI_MAX_MULTI_SIZE == 8)
	    if(node::blindileaftype == 8)
	    {
		return 0;		
	    }
            return 1;
...
#ifdef BREATHING_SIZE
	/// True if the node's need to breath
        inline bool isneedbreath() const
        {
            return (node::slotuse == node::currentmaxslot);
        }
#else 
        inline bool isneedbreath() const
        {
    ...
    /// data items. Key and data slots are kept in separate arrays, because the
    /// key array is traversed very often compared to accessing the data items.
    struct btree_leaf_node : public leaf_node
    {
        /// Define an related allocator for the leaf_node structs.
    ...
    /// Extended structure of a leaf node in memory for blindi. Contains pairs of keys and
    /// data items. Key and data slots are kept in separate arrays, because the
    /// key array is traversed very often compared to accessing the data items.
    struct blindi_leaf_node1 : public leaf_node
    {
        /// Define an related allocator for the leaf_node structs.
        typedef typename _Alloc::template rebind<blindi_leaf_node1>::other alloc_type;

        blindi_node1 blindi_hybrid_node;
    /// Set variables to initial values
        inline void initialize()
        {
            leaf_node::initialize(1);
        }
    };
    struct blindi_leaf_node2 : public leaf_node
    {
        /// Define an related allocator for the leaf_node structs.
        typedef typename _Alloc::template rebind<blindi_leaf_node2>::other alloc_type;

        blindi_node2 blindi_hybrid_node;
    /// Set variables to initial values
        inline void initialize()
        {
            leaf_node::initialize(2);
        }
    };
    struct blindi_leaf_node4 : public leaf_node
    {
        /// Define an related allocator for the leaf_node structs.
        typedef typename _Alloc::template rebind<blindi_leaf_node4>::other alloc_type;

        blindi_node4 blindi_hybrid_node;
    /// Set variables to initial values
        inline void initialize()
        {
            leaf_node::initialize(4);
        }
    };
    struct blindi_leaf_node8 : public leaf_node
    {
        /// Define an related allocator for the leaf_node structs.
        typedef typename _Alloc::template rebind<blindi_leaf_node8>::other alloc_type;

        blindi_node8 blindi_hybrid_node;
    /// Set variables to initial values
        inline void initialize()
        {
            leaf_node::initialize(8);
        }
    };
 


// Varibales for the Hybrid process
/*
The comppression defines by the maximum compression we can get (Blindi16, Blindi128, Blindi128Breath, Blindi512Breath)
Compress during tagged insert :
From Btree 16 we compress to Blindi 16.
From two Blindi neighbors of Blindi 16, we merge to Blindi 32.  
From one Blindi neighbor of 16 and 32 try  merge to Blindi 32/64.    
From one Blindi neighbor of 16/32/64 and 32/64 try to merge to Blindi 64/128. 

Compress during split:
From Btree 16 we create one Blindi 32.  

If we are not in "may uncompress" we create larger blindi instead of split.
From Btree 16 we create Blindi 32.
From Blindi 16, we create Blindi 32.  
From Blindi 32, we create Blindi 64.    
From Blindi 64, we create Blindi 128


Uncompress during tagged remove: 
split Blindi 128 to blindi 64
split Blindi 64 to blindi 32
split Blindi 32 to blindi 16
transfer Blindi 16 to Btree 16  

The algorithm:
We start with Btree 16 in "continue" mode
When the capacity reaches Y%*FULL_CAPACITY than we move to  "should_compress"
When the capacity reaches 100%*X than we move to  "Must compress"  
From "May Compress" we can move to "continue" if we lower down  to 95%X
When reaching to 80% and have a number of items less than the number that cause us the first "should compress" we start to uncompress. 
*/
...
private: 
    enum hybrid_status
    {
       CONTINUE = 0, // continue the current status without further compress or uncompress, If split from blindi continue with blindi and with Btree continue with BTREE 
       MAY_UNCOMPRESS = 1, // may uncompress if the all leaves with the same parent are all BTREE"
       
       SHOULD_COMPRESS = 2, // should compress one leaf from the same parent. prefrence to the leaf we are working on. 
       MUST_COMPRESS = 3 // WE are very near to the END - compress every insert
    };
...
private:
    ...
    struct blindi_btree_hybrid_nodes_pair_to_value
    {
    ...
    struct blindi_btree_hybrid_nodes_pair_to_value<value_type, value_type>
    {
    ...
    /// iterators
    typedef blindi_btree_hybrid_nodes_pair_to_value<value_type, pair_type> pair_to_value_type;
...
public:
    ...
    public:
        ...
        /// The key type of the blindi_btree_hybrid_nodes. Returned by key().
        typedef typename blindi_btree_hybrid_nodes::key_type                key_type;

        /// The data type of the blindi_btree_hybrid_nodes. Returned by data().
        typedef typename blindi_btree_hybrid_nodes::data_type               data_type;

        /// The value type of the blindi_btree_hybrid_nodes. Returned by operator*().
        typedef typename blindi_btree_hybrid_nodes::value_type              value_type;

        /// The pair type of the blindi_btree_hybrid_nodes.
        typedef typename blindi_btree_hybrid_nodes::pair_type               pair_type;
    ...
    public:
        ...
        typename blindi_btree_hybrid_nodes::leaf_node*      currnode;
        ...
        /// Also friendly to the base blindi_btree_hybrid_nodes class, because erase_iter() needs
        /// to read the currnode and currslot values directly.
        friend class blindi_btree_hybrid_nodes<key_type, data_type, BlindiBtreeHybridType,  _InnerSlots, _LeafSlots, value_type, key_compare,
    ...
    public:
        ...
        inline iterator(typename blindi_btree_hybrid_nodes::leaf_node *l, unsigned short s)
	...
	inline uint8_t * get_key_blindi(leaf_node *currnode, int currslot) const
	{
		switch (currnode->blindileaftype) 
	    {
		    case 1:	{ 
					blindi_leaf_node1 *blindi_leaf1 = static_cast<blindi_leaf_node1*>(currnode);
					return blindi_leaf1->blindi_hybrid_node.get_key_ptr(currslot);
				}
		    case 2:	{ 
					blindi_leaf_node2 *blindi_leaf2 = static_cast<blindi_leaf_node2*>(currnode);
					return blindi_leaf2->blindi_hybrid_node.get_key_ptr(currslot);
				}
		    case 4:	{ 
					blindi_leaf_node4 *blindi_leaf4 = static_cast<blindi_leaf_node4*>(currnode);
					return blindi_leaf4->blindi_hybrid_node.get_key_ptr(currslot);
				}
		    case 8:	{ 
					blindi_leaf_node8 *blindi_leaf8 = static_cast<blindi_leaf_node8*>(currnode);
					return blindi_leaf8->blindi_hybrid_node.get_key_ptr(currslot);
				}
		    default :
                std::cout << "Error: blindileaftype is " << currnode->blindileaftype << std::endl;
                getchar();	            
	    }
	}

	inline data_type& get_keyptr_blindi(leaf_node *currnode, int currslot)
	{
        uint8_t** ptr;
		switch (currnode->blindileaftype) 
	    {
		    case 1:	{ 
					blindi_leaf_node1 *blindi_leaf1 = static_cast<blindi_leaf_node1*>(currnode);
					ptr = blindi_leaf1->blindi_hybrid_node.get_ptr2key_ptr();
                    break;
				}
		    case 2:	{ 
					blindi_leaf_node2 *blindi_leaf2 = static_cast<blindi_leaf_node2*>(currnode);
					ptr = blindi_leaf2->blindi_hybrid_node.get_ptr2key_ptr();
                    break;
				}
		    case 4:	{ 
					blindi_leaf_node4 *blindi_leaf4 = static_cast<blindi_leaf_node4*>(currnode);
					ptr = blindi_leaf4->blindi_hybrid_node.get_ptr2key_ptr();
                    break;
				}
		    case 8:	{ 
					blindi_leaf_node8 *blindi_leaf8 = static_cast<blindi_leaf_node8*>(currnode);
					ptr = blindi_leaf8->blindi_hybrid_node.get_ptr2key_ptr();
                    break;
				}
		    default :
                std::cout << "Error: blindileaftype is " << currnode->blindileaftype << std::endl;
		...
		{
			return *((key_type*)get_key_blindi(currnode, currslot));
		}
		else 
		{
			btree_leaf_node *btree_leaf = static_cast<btree_leaf_node*>(currnode);
			return btree_leaf->slotkey[currslot];
		}
        }
		...
		{
			btree_leaf_node *btree_leaf = static_cast<btree_leaf_node*>(currnode);
			return btree_leaf->slotdata[used_as_set ? 0 : currslot];
		}
        }
		...
		if(currnode->isblindileafnode())
		{
			return get_keyptr_blindi(currnode, currslot);
		}
		else 
		{
			btree_leaf_node *btree_leaf = static_cast<btree_leaf_node*>(currnode);
...
//                currslot = currnode->slotuse;
    ...
    private:
        ...
        /// The currently referenced leaf node of the tree
        const typename blindi_btree_hybrid_nodes::leaf_node*        currnode;
    ...
    public:
        ...
        inline const_iterator(const typename blindi_btree_hybrid_nodes::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
        ...
        inline const key_type& key() const
        {
            if(currnode->level == 0 && currnode->isblindileafnode()){
		    return *((key_type*)get_key_blindi(currnode, currslot));
	    }	
    ...
    public:
        ...
        inline reverse_iterator(typename blindi_btree_hybrid_nodes::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
        ...
        {
            BTREE_ASSERT(currslot > 0);
            if(currnode->level == 0 && currnode->sblindileafnode()){
		    return *((key_type*)get_key_blindi(currnode, currslot - 1));
	    }	
        ...
        inline data_type& data() const
        {
            BTREE_ASSERT(currslot > 0);
            if(currnode->level == 0 && currnode->isblindileafnode()){
		    return (data_type) get_key_blindi(currnode, currslot - 1);
	    }	
            return currnode->slotkey[currslot - 1];
        }
    ...
    public:
        ...
        inline const_reverse_iterator(const typename blindi_btree_hybrid_nodes::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
...
public:
    ...
    {
        // For hybrid support   
        hybrid_status hybridstatus; // The current status
        size_type  indexcapacity; // The current size of the Index 
        size_type  limitArrived; // The MAX limit
        size_type  limit2MustCompress; //  
        size_type  limit2ShouldCompress; // 
        size_type  limit2ReturmForUnCompress; //
        size_type  limit2ReturmFromShouldCompress; // 
        size_type  firsttimeCompressitemscount;
        bool isNotFirstrsttimeCompress;
...
#ifdef BREATHING_SIZE 
        /// Number of slot size whenn we have breathing node
	size_type     leavesslotsbreath;  
	size_type     breathing_num;  
#endif 
        /// Number of leaves in the B+ tree
        size_type       leaves;

        /// Number of leaves type btree in the B+ tree
        size_type       btreeleaves;

        /// Number of leaves type blindi the B+ tree
        size_type       blindileaves;

        /// Number of inner nodes in the B+ tree
        size_type       innernodes;

        /// Number of transfer of blindi node to btree node
        size_type       blindi2btreetrans;
        
	/// Number of transfer of btree node to blindi node
        size_type       btree2blindi1trans;
        size_type       btree2blindi2trans;

	/// Number of transfer of blindi_nodeX to blindi_nodeY
	size_type       blindi1to2trans;
	size_type       blindi2to4trans;
	size_type       blindi4to8trans;

	/// Number of transfer of blindi_nodeX to blindi_nodeY
	size_type       blindinodes1;
	size_type       blindinodes2;
	size_type       blindinodes4;
	size_type       blindinodes8;
        /// Base B+ tree parameter: The number of key/data slots in each leaf
        static const unsigned short     leafslots = blindi_btree_hybrid_nodes_self::leafslotmax;

        /// Base B+ tree parameter: The number of key slots in each inner node.
        static const unsigned short     innerslots = blindi_btree_hybrid_nodes_self::innerslotmax;

        static const size_t             leafsize = sizeof(leaf_node);
        static const size_t             innersize = sizeof(inner_node);
        static const size_t             btreeleafsize = sizeof(btree_leaf_node);
        static const size_t             blindileafsize = sizeof(blindi_leaf_node1);
        static const size_t             nodesize = sizeof(inner_node);

        /// Zero initialized
        inline tree_stats()

            : hybridstatus(CONTINUE),
              indexcapacity(0),
//              itemcount(0),
              limitArrived(uint64_t(LIMIT_CAPACITY)),
              limit2MustCompress(uint64_t(LIMIT_CAPACITY /100 * MUST_PRECENT)),
              limit2ShouldCompress (uint64_t(LIMIT_CAPACITY /100 * SHOULD_COMPRESS_PERCENT)),
              limit2ReturmForUnCompress (uint64_t(LIMIT_CAPACITY /100 * 0)),
              limit2ReturmFromShouldCompress(uint64_t(LIMIT_CAPACITY /100 * 0)),
              firsttimeCompressitemscount(0),		 
              isNotFirstrsttimeCompress(0),
#ifdef BREATHING_SIZE 
	    leavesslotsbreath(0),  
	    breathing_num(0),  
#endif 
              leaves(0), btreeleaves(0),  blindileaves(0), innernodes(0), blindi2btreetrans(0), btree2blindi1trans(0), blindi1to2trans(0), blindi2to4trans(0), blindi4to8trans(0),blindinodes1(0), blindinodes2(0), blindinodes4(0), blindinodes8(0)
        {
        }
	...
	/// is the insert is a triggered insert
        inline double istrigeredinsert() const 
        {
            return ((itemcount % minleafslots) == 0);
        }


#ifdef BREATHING_SIZE 
        /// Return the average size of leaves
        inline double avgfill_leavesslotbreath() const
        {
            return static_cast<double>(leavesslotsbreath) / (leaves);
	} 
#endif 
...
public:
    ...
    /// comparison function
    explicit inline blindi_btree_hybrid_nodes(const allocator_type &alloc = allocator_type())
    ...
    /// comparison object
    explicit inline blindi_btree_hybrid_nodes(const key_compare &kcf,
    ...
    inline blindi_btree_hybrid_nodes(InputIterator first, InputIterator last,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_allocator(alloc)
    {
        insert(first, last);
    ...
    template <class InputIterator>
    inline blindi_btree_hybrid_nodes(InputIterator first, InputIterator last, const key_compare &kcf,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_key_less(kcf), m_allocator(alloc)
    {
    ...
    inline ~blindi_btree_hybrid_nodes()
    {
        clear();
    }

    /// Fast swapping of two identical B+ tree objects.
    void swap(blindi_btree_hybrid_nodes_self& from)
    {
...
public:
    ...
    protected:
        ...
        /// Constructor called from blindi_btree_hybrid_nodes::value_comp()
        inline value_compare(key_compare kc)
            : key_comp(kc)
        { }

        /// Friendly to the blindi_btree_hybrid_nodes class so it may call the constructor
        friend class blindi_btree_hybrid_nodes<key_type, data_type, BlindiBtreeHybridType, _InnerSlots, _LeafSlots, value_type, key_compare,
                           traits, allow_duplicates, allocator_type, used_as_set>;
...
private:
    ...
    inline bool key_lessequal(const uint8_t* a, const uint8_t* b) const
    {
        return !m_key_less(*((key_type*)b), *((key_type*)a));
    }
    ...
    inline bool key_greater(const key_type &a, const uint8_t* b) const
    {
        return m_key_less(*((key_type*)b), a);
    }
...
private:
    ...
    /// Return an allocator for btree_leaf_node objects
    typename btree_leaf_node::alloc_type btree_leaf_node_allocator()
    {
        return typename btree_leaf_node::alloc_type(m_allocator);
    }

    /// Return an allocator for btree_leaf_node objects
    typename blindi_leaf_node1::alloc_type blindi_leaf_node1_allocator()
    {
        return typename blindi_leaf_node1::alloc_type(m_allocator);
    }

    /// Return an allocator for btree_leaf_node objects
    typename blindi_leaf_node2::alloc_type blindi_leaf_node2_allocator()
    {
        return typename blindi_leaf_node2::alloc_type(m_allocator);
    }

    /// Return an allocator for btree_leaf_node objects
    typename blindi_leaf_node4::alloc_type blindi_leaf_node4_allocator()
    {
        return typename blindi_leaf_node4::alloc_type(m_allocator);
    }

    /// Return an allocator for btree_leaf_node objects
    typename blindi_leaf_node8::alloc_type blindi_leaf_node8_allocator()
    {
        return typename blindi_leaf_node8::alloc_type(m_allocator);
    }
    ...
    inline btree_leaf_node* allocate_btree_leaf(int slotsize)
    {
        btree_leaf_node *n = new (btree_leaf_node_allocator().allocate(1)) btree_leaf_node();
#ifdef BREATHING_SIZE_BTREE
#ifndef BREATHING_DATA_ONLY_BTREE
        n->slotkey = (key_type *)malloc (slotsize * sizeof(key_type));  
#endif 
        n->slotdata = (data_type *)malloc (slotsize * sizeof(data_type));  
        n->currentmaxslot = slotsize; 
#endif
        n->initialize();
        m_stats.leaves++;
        m_stats.btreeleaves++;
        m_stats.indexcapacity += sizeof(*n);
//        std::cout << "size of btree_leaf_node " << sizeof(*n) <<  std::endl; 
        return n;
    }

    /// Allocate and initialize a leaf node
    inline blindi_leaf_node1* allocate_blindi_leaf1()
    {
        m_stats.leaves++;
        m_stats.blindileaves++;
	blindi_leaf_node1 *n = new blindi_leaf_node1();
	m_stats.blindinodes1++;
	n->initialize();
	m_stats.indexcapacity += sizeof(*n); 
//        std::cout << "size of blindinodes1 " << sizeof(*n) <<  std::endl; 
        return n;
    }
    inline blindi_leaf_node2* allocate_blindi_leaf2()
    {
        m_stats.leaves++;
        m_stats.blindileaves++;
	blindi_leaf_node2 *n = new blindi_leaf_node2();
	m_stats.blindinodes2++;
	n->initialize();
	m_stats.indexcapacity += sizeof(*n); 
//        std::cout << "size of blindinodes2 " << sizeof(*n) <<  std::endl; 
        return n;
    }
    inline blindi_leaf_node4* allocate_blindi_leaf4()
    {
        m_stats.leaves++;
        m_stats.blindileaves++;
	blindi_leaf_node4 *n = new blindi_leaf_node4();
	m_stats.blindinodes4++;
	n->initialize();
	m_stats.indexcapacity += sizeof(*n); 
//        std::cout << "size of blindinodes4 " << sizeof(*n) <<  std::endl; 
        return n;
    }
    inline blindi_leaf_node8* allocate_blindi_leaf8()
    {
        m_stats.leaves++;
        m_stats.blindileaves++;
	blindi_leaf_node8 *n = new blindi_leaf_node8();
	m_stats.blindinodes8++;
	n->initialize();
	m_stats.indexcapacity += sizeof(*n); 
//        std::cout << "size of blindinodes8 " << sizeof(*n) <<  std::endl; 
        return n;
    }
    ...
    inline inner_node* allocate_inner(unsigned short level)
    {
        ...
        m_stats.innernodes++;
        m_stats.indexcapacity += sizeof(*n);  
//        std::cout << "size of inner " << sizeof(*n) <<  std::endl; 
        return n;
    }
    ...
    {
        if (n->isleafnode()) {
		leaf_node *leaf_tmp = static_cast<btree_leaf_node*>(n);
		if (!leaf_tmp->isblindileafnode()) {
		    btree_leaf_node *ln = static_cast<btree_leaf_node*>(n);
		    typename btree_leaf_node::alloc_type a(btree_leaf_node_allocator());
		    m_stats.indexcapacity -= sizeof(*ln);  
		    a.destroy(ln);
		    a.deallocate(ln, 1);
		    m_stats.btreeleaves--;
		}
		...
		{
			m_stats.blindileaves--;
                        switch (leaf_tmp->blindileaftype) 
			{
				case 1:
				{       
					       blindi_leaf_node1 *ln = static_cast<blindi_leaf_node1*>(n);
					       typename blindi_leaf_node1::alloc_type a(blindi_leaf_node1_allocator());
					       m_stats.indexcapacity -= sizeof(*ln);
					       a.destroy(ln);
					       a.deallocate(ln, 1);
					       m_stats.blindinodes1--;
				...
				case 2:
				{       
					       blindi_leaf_node2 *ln = static_cast<blindi_leaf_node2*>(n);
					       typename blindi_leaf_node2::alloc_type a(blindi_leaf_node2_allocator());
					       m_stats.indexcapacity -= sizeof(*ln);
					       a.destroy(ln);
					       a.deallocate(ln, 1);
					       m_stats.blindinodes2--;
				...
				case 4:
				{       
					       blindi_leaf_node4 *ln = static_cast<blindi_leaf_node4*>(n);
					       typename blindi_leaf_node4::alloc_type a(blindi_leaf_node4_allocator());
					       m_stats.indexcapacity -= sizeof(*ln);
					       a.destroy(ln);
					       a.deallocate(ln, 1);
					       m_stats.blindinodes4--;
				...
				{     
					       ...
					       blindi_leaf_node8 *ln = static_cast<blindi_leaf_node8*>(n);
					       typename blindi_leaf_node8::alloc_type a(blindi_leaf_node8_allocator());
					       m_stats.indexcapacity -= sizeof(*ln);
					       a.destroy(ln);
					       a.deallocate(ln, 1);
					       m_stats.blindinodes8--;
		    ...
		    default :
                std::cout << "Error: blindileaftype is " << leaf_tmp->blindileaftype << std::endl;
                getchar();	    
			}
		}		
		m_stats.leaves--;
...
private:
    ...
    {
      //	std::cout << "clear_recursive " << std::endl;
        if (n->isleafnode())
        {
            btree_leaf_node *leafnode = static_cast<btree_leaf_node*>(n);

            for (unsigned int slot = 0; slot < leafnode->slotuse; ++slot)
            {
                // data objects are deleted by btree_leaf_node's destructor
            }
        }
        else
        {
...
private:
    ...
    inline int find_lower(const node_type *n, const key_type& key) const
    {
        if ( 0 && sizeof(n->slotkey) > traits::binsearch_threshold )
        {
            ...
            BTREE_PRINT("blindi_btree_hybrid_nodes::find_lower: on " << n << " key " << key << " -> " << lo << " / " << hi);
            ...
            if (selfverify)
            {
                ...
                BTREE_PRINT("blindi_btree_hybrid_nodes::find_lower: testfind: " << i);
                BTREE_ASSERT(i == lo);
            }
    ...
    inline int find_upper(const node_type *n, const key_type& key) const
    {
        if ( 0 && sizeof(n->slotkey) > traits::binsearch_threshold )
        {
            ...
            BTREE_PRINT("blindi_btree_hybrid_nodes::find_upper: on " << n << " key " << key << " -> " << lo << " / " << hi);
            ...
            if (selfverify)
            {
                ...
                BTREE_PRINT("blindi_btree_hybrid_nodes::find_upper testfind: " << i);
                BTREE_ASSERT(i == hi);
            }
...
public:
    ...
    /// Return the current capacity status
    inline size_type capacity() const
    {
        return m_stats.indexcapacity;
    }
...
public:
    ...
    static inline uint8_t * get_key_blindi_static(leaf_node *currnode, int currslot)
    {
	    switch (currnode->blindileaftype) 
	    {
		    case 1:	{ 
    ...
    inline int search_blindi(leaf_node *currnode, const key_type &key, int search_type, bool *hit, bool *smaller_than_node, uint16_t *tree_traverse, uint16_t* tree_traverse_len) const
    {
	    switch (currnode->blindileaftype) 
	    {
		    case 1:	{
					blindi_leaf_node1 *blindi_leaf1 = static_cast<blindi_leaf_node1*>(currnode);
					return blindi_leaf1->blindi_hybrid_node.SearchBlindiNode((uint8_t*) &key, _Key_len, search_type, hit, smaller_than_node, tree_traverse, tree_traverse_len);
				}
		    case 2:	{ 
					blindi_leaf_node2 *blindi_leaf2 = static_cast<blindi_leaf_node2*>(currnode);
					return blindi_leaf2->blindi_hybrid_node.SearchBlindiNode((uint8_t*) &key, _Key_len, search_type, hit, smaller_than_node, tree_traverse, tree_traverse_len);
				}
		    case 4:	{ 
					blindi_leaf_node4 *blindi_leaf4 = static_cast<blindi_leaf_node4*>(currnode);
					return blindi_leaf4->blindi_hybrid_node.SearchBlindiNode((uint8_t*) &key, _Key_len, search_type, hit, smaller_than_node, tree_traverse, tree_traverse_len);
				}
		    case 8:	{ 
					blindi_leaf_node8 *blindi_leaf8 = static_cast<blindi_leaf_node8*>(currnode);
					return blindi_leaf8->blindi_hybrid_node.SearchBlindiNode((uint8_t*) &key, _Key_len, search_type, hit, smaller_than_node, tree_traverse, tree_traverse_len);
				}
		    default :
                std::cout << "Error: blindileaftype is " << currnode->blindileaftype << std::endl;
                getchar();	    
...
//					std::cout << "Insert blindi_leaf_node8 " << std::endl;
    ...
    inline void split_blindi_nodex(leaf_node *currnode, key_type* splitkey, node ** splitnode)
    {
	    switch (currnode->blindileaftype) 
	    {
		    case 1:	{ 
					blindi_leaf_node1 *blindi_leaf1 = static_cast<blindi_leaf_node1*>(currnode);
					split_blindi_leaf_node1(blindi_leaf1, splitkey, splitnode);
                                        break;  
				}
		    case 2:	{ 
					blindi_leaf_node2 *blindi_leaf2 = static_cast<blindi_leaf_node2*>(currnode);
					split_blindi_leaf_node2(blindi_leaf2, splitkey, splitnode);
                                        break;  
				}
		    case 4:	{ 
					blindi_leaf_node4 *blindi_leaf4 = static_cast<blindi_leaf_node4*>(currnode);
					split_blindi_leaf_node4(blindi_leaf4, splitkey, splitnode);
                                        break;  
				}
		    case 8:	{ 
					blindi_leaf_node8 *blindi_leaf8 = static_cast<blindi_leaf_node8*>(currnode);
					split_blindi_leaf_node8(blindi_leaf8, splitkey, splitnode);
                                        break;  
				}
		    default :
                std::cout << "Error: blindileaftype is " << currnode->blindileaftype << std::endl;
...
//        std::cout << " end search blindi " << std::endl;    
	...
	bool hit = false, smaller_than_node = false;
        uint16_t tree_traverse[5];
        uint16_t tree_traverse_len;
	int slot = search_blindi(leaf, key, SEARCH_TYPE::PREDECESSOR_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);
        if (hit || smaller_than_node)  {
		return iterator(leaf, slot);
	}
        if(leaf == m_tailleaf && (slot == leaf->slotuse - 1)) {
   	    return end();
	}
	return iterator(leaf, slot++);
...
public:
    ...
    inline bool operator==(const blindi_btree_hybrid_nodes_self &other) const
    {
...
public:
    ...
    inline blindi_btree_hybrid_nodes_self& operator= (const blindi_btree_hybrid_nodes_self &other)
    {
        if (this != &other)
        {
            clear();
    ...
    inline blindi_btree_hybrid_nodes(const blindi_btree_hybrid_nodes_self &other)
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_stats( other.m_stats ),
          m_key_less( other.key_comp() ),
          m_allocator( other.get_allocator() )
...
private:
    ...
    struct node* copy_recursive(const node *n)
    {
        ...
        {
            const leaf_node *leaf = static_cast<const leaf_node*>(n);
            leaf_node *newleaf = allocate_btree_leaf(leafslotmax);
...
//		m_root = m_headleaf = m_tailleaf = allocate_blindi_leaf1();
	...
	bool istransferbtreeblindi = 0;
        ...
        if (newchild)
        {
            ...
            m_root = newroot;
	    m_stats.limit2MustCompress -= sizeof(*newroot);	
        }
...
#endif
//	print_node(std::cout, m_root, 0, true);
        if (selfverify) {
            verify();
            BTREE_ASSERT(exists(key));
        }

        uint64_t capacity = m_stats.indexcapacity;

#ifdef BREATHING_BLINDI_STSTS
		capacity += sizeof(uint8_t*) * breathing_sum;
        ...
        if (0)
	{           
		if (capacity >= m_stats.limitArrived )
		{
			std::cout << "LIMIT_ARRIVED limit: " << m_stats.limitArrived << " capacity: " <<  capacity << "  itemcount "  << m_stats.itemcount << " shold_compress " <<  m_stats.firsttimeCompressitemscount <<  std::endl;
			exit(0);
		}
	}
	switch (m_stats.hybridstatus)
	{
		case CONTINUE :
#ifdef IS_ITEMSCOUNT 
			if ((m_stats.btreeleaves) > 0 &&  (m_stats.itemcount > m_stats.limit2ShouldCompress))
#else
			if ((m_stats.btreeleaves) > 0 &&  (capacity > m_stats.limit2ShouldCompress))
#endif
			{
                                if(MUST_ONLY) 
				{
					m_stats.hybridstatus = MUST_COMPRESS;
				}
				else
				{
					m_stats.hybridstatus = SHOULD_COMPRESS;
				}
				if (m_stats.isNotFirstrsttimeCompress == 0)
				{
					m_stats.isNotFirstrsttimeCompress = 1;
					m_stats.firsttimeCompressitemscount = m_stats.itemcount;
				}
			}
                        break;
		case MAY_UNCOMPRESS : 
#ifdef IS_ITEMSCOUNT 
			if (m_stats.itemcount > m_stats.limit2ReturmForUnCompress ) 
#else
			if (capacity > m_stats.limit2ReturmForUnCompress ) 
#endif
			{
				m_stats.hybridstatus = CONTINUE;
			}
			break;
		case SHOULD_COMPRESS:
#ifdef IS_ITEMSCOUNT 
			if (m_stats.itemcount > m_stats.limit2MustCompress ) {
#else
			if (capacity > m_stats.limit2MustCompress ) {
#endif
				m_stats.hybridstatus = MUST_COMPRESS;
			}
			else if (m_stats.indexcapacity < m_stats.limit2ReturmFromShouldCompress) {
				m_stats.hybridstatus = CONTINUE;
			}
			break;

		case MUST_COMPRESS: 
			if (m_stats.btreeleaves == 0) 
			{
				m_stats.hybridstatus = CONTINUE;
			}
//			if (capacity < m_stats.limit2ShouldCompress)
//			{
//				m_stats.hybridstatus = SHOULD_COMPRESS;
//			}	
	}
...
//            std::cout << " insert descent no leaf node " << std::endl; 
            ...
            if (newchild)
            {
                BTREE_PRINT("blindi_btree_hybrid_nodes::insert_descend newchild with key " << newkey << " node " << newchild << " at slot " << slot);

                if (inner->isfull())
                {
                    split_inner_node(inner, splitkey, splitnode, slot);

                    BTREE_PRINT("blindi_btree_hybrid_nodes::insert_descend done split_inner: putslot: " << slot << " putkey: " << newkey << " upkey: " << *splitkey);
                    ...
                    BTREE_PRINT("blindi_btree_hybrid_nodes::insert_descend switch: " << slot << " > " << inner->slotuse+1);
                    ...
                    else if (slot >= inner->slotuse+1)
                    {
                        ...
                        BTREE_PRINT("blindi_btree_hybrid_nodes::insert_descend switching to splitted node " << inner << " slot " << slot);
                    }
                }
...
//            std::cout << " insert to leaf node " << std::endl; 
	    ...
	    // transfer btreenode to blindinode
            if (!leaf_tmp->isblindileafnode()) 
	    {
//		    if ((m_stats.hybridstatus == MUST_COMPRESS) || ((m_stats.hybridstatus == SHOULD_COMPRESS) && ((m_stats.istrigeredinsert()) || leaf_tmp->isfullbtree()))) 
		    if ((m_stats.hybridstatus == MUST_COMPRESS) || ((m_stats.hybridstatus == SHOULD_COMPRESS) && leaf_tmp->isfullbtree())) 
		    {
#if (BLINDI_MAX_MULTI_SIZE == 1)
			    leaf_tmp = transfer_btree_node_blindi1_node(n, parent_n, parentslot);
#endif
#if (BLINDI_MAX_MULTI_SIZE > 1)
			    if (!leaf_tmp->isfullbtree()) {
				    leaf_tmp = transfer_btree_node_blindi1_node(n, parent_n, parentslot);
			    }
			    else {	
				    leaf_tmp = transfer_btree_node_blindi2_node(n, parent_n, parentslot);
			    }
	    ...
	    {
                    // go over all the leaves of the same parent and compress the first one that is BTREE 
                    if(1) 
		    { 	
			    inner_node *inner = static_cast<inner_node*>(parent_n);
			    if ((m_stats.hybridstatus == MUST_COMPRESS) && (!inner->isalreadycomprerss()))
			    {
				    bool iscompress = 0;
				    for (int i=0; i < parent_n->slotuse; i++)
				    {
					    leaf_node *leaf = static_cast<leaf_node*>(inner->childid[i]);
					    if(!leaf->isblindileafnode()) 
					    {
						    transfer_btree_node_blindi1_node(leaf, parent_n, i);
						    iscompress = 1;
						    break;
					    }
				    }
				    if(!iscompress) {
//					    std::cout << "inner->alreadycomprerss = 1 " << std::endl;
					    inner->alreadycomprerss = 1;
				    }
		    ...
		    if(0) 
		    {  
			    btree_leaf_node *newibtreeleaf = allocate_btree_leaf(leafslotmax);
			    blindi_leaf_node1 *leaf_blindi = static_cast<blindi_leaf_node1*>(n);
			    transfer_blindi_node_btree_node(leaf_blindi, newibtreeleaf);
			    // connect the tree to the new node
			    if (n != parent_n) // if we are not transfering the root
			    {
				    inner_node *inner = static_cast<inner_node*>(parent_n);
				    inner->childid[parentslot] = newibtreeleaf;
				    // cange the parent pointer/ change the left pointer and the right pointer
				    if(leaf_blindi->prevleaf) 
				    { 	
					    leaf_blindi->prevleaf->nextleaf = newibtreeleaf;
				    }	
				    if(leaf_blindi->nextleaf)
				    { 
					    leaf_blindi->nextleaf->prevleaf = newibtreeleaf;
				    }
			    }
			    free_node(leaf_blindi);
			    leaf_tmp = newibtreeleaf;
			    m_stats.blindi2btreetrans++;	
		    }
	    }
         
            /* IMPORTANT: We currently DO NOT support duplicated keys
...
//		    std:: cout << "Insert to BTREE slot " << slot << std::endl;
		    ...
		    {
			    // regular split
			    split_btree_leaf_node(leaf, splitkey, splitnode);

			    // check if insert slot is in the split sibling node
			    if (slot >= leaf->slotuse)
			    {
				    slot -= leaf->slotuse;
				    leaf = static_cast<btree_leaf_node*>(*splitnode);
			    }
		    } 
#ifdef BREATHING_SIZE_BTREE 
		    else if (leaf->isneedbreath()) 
		    {
			    if (leaf->currentmaxslot + BREATHING_SIZE > leafslotmax) 
			    {
				    m_stats.leavesslotsbreath += leafslotmax - leaf->currentmaxslot ;
				    leaf->currentmaxslot = leafslotmax;
			    }
			    else 
			    {
				    m_stats.leavesslotsbreath += BREATHING_SIZE;
				    leaf->currentmaxslot += BREATHING_SIZE;
			    }
			    m_stats.breathing_num ++;
                     
#ifndef BREATHING_DATA_ONLY_BTREE
			    key_type *tmp_key_p = (key_type *)malloc (leaf->currentmaxslot * sizeof(key_type));   
			    std::copy(leaf->slotkey, leaf->slotkey + leaf->slotuse,
					    tmp_key_p);
			    free(leaf->slotkey);
			    leaf->slotkey = tmp_key_p;
#endif  
			    data_type * tmp_data_p = (data_type *)malloc (leaf->currentmaxslot * sizeof(data_type));
			    std::copy(leaf->slotdata, leaf->slotdata + leaf->slotuse,
					    tmp_data_p);
			    free(leaf->slotdata);
			    leaf->slotdata = tmp_data_p;
		    }
#endif
...
//			    std::cout << "leaf->isblindi_can_extend " << (int)leaf->isblindi_can_extend() << std::endl;
			    ...
			    {
				    b_split = true;
				    split_blindi_nodex(leaf_tmp, splitkey, splitnode);
				    if (key_greater(key, *splitkey))
				    {
		    ...
		    if (b_split && leaf != *splitnode && key_greater(key, get_key_blindi(leaf, leaf->blindileaftype * leafslotmax/2 - 1)))
		    {
			    // special case: the node was split, and the insert is at the
			    // last slot of the old node. then the splitkey must be
			    // updated.
...
//                   If not supported duplication
//		    if (insert_blindi(leaf, key) == INSERT_DUPLICATED_KEY){
//			    return std::pair<iterator, bool>(iterator(leaf, slot), false);
//		    }
		    insert_blindi(leaf, key);
		    leaf->slotuse++;
...
//	    std::cout  << " transfer_blindixtoblibdi2x " << std::endl;
	    ...
	    {
		    case 1 : {
				     blindi_leaf_node2 *newblindileaf = allocate_blindi_leaf2();
				     blindi_leaf_node1 *oldblindileaf = static_cast<blindi_leaf_node1*>(leaf_old);
				     newblindileaf->blindi_hybrid_node.node.transferNode(oldblindileaf->blindi_hybrid_node.node.get_valid_len(), oldblindileaf->blindi_hybrid_node.node.get_ptr2blindiKeys(), oldblindileaf->blindi_hybrid_node.node.get_ptr2key_ptr(), oldblindileaf->blindi_hybrid_node.node.get_ptr2blindiTree()); 
				     leaf_node *leaf_new = static_cast<leaf_node*>(newblindileaf);

				     connect_blindi2x(leaf_old, leaf_new, parent_n, parentslot);
				     m_stats.blindi1to2trans++;
				     free_node(oldblindileaf);
				     return leaf_new; 
			     }
		    case 2: {
				    blindi_leaf_node4 *newblindileaf = allocate_blindi_leaf4();
				    blindi_leaf_node2 *oldblindileaf = static_cast<blindi_leaf_node2*>(leaf_old);
				    newblindileaf->blindi_hybrid_node.node.transferNode(oldblindileaf->blindi_hybrid_node.node.get_valid_len(), oldblindileaf->blindi_hybrid_node.node.get_ptr2blindiKeys(), oldblindileaf->blindi_hybrid_node.node.get_ptr2key_ptr(), oldblindileaf->blindi_hybrid_node.node.get_ptr2blindiTree());
				     leaf_node *leaf_new = static_cast<leaf_node*>(newblindileaf);
				     connect_blindi2x(leaf_old, leaf_new, parent_n, parentslot);
				     m_stats.blindi2to4trans++;
				     free_node(oldblindileaf);
				     return leaf_new; 
			    }
		    case 4:{
				   blindi_leaf_node8 *newblindileaf = allocate_blindi_leaf8();
				   blindi_leaf_node4 *oldblindileaf = static_cast<blindi_leaf_node4*>(leaf_old);
				   newblindileaf->blindi_hybrid_node.node.transferNode(oldblindileaf->blindi_hybrid_node.node.get_valid_len(), oldblindileaf->blindi_hybrid_node.node.get_ptr2blindiKeys(), oldblindileaf->blindi_hybrid_node.node.get_ptr2key_ptr(), oldblindileaf->blindi_hybrid_node.node.get_ptr2blindiTree());
				   leaf_node *leaf_new = static_cast<leaf_node*>(newblindileaf);
				   connect_blindi2x(leaf_old, leaf_new, parent_n, parentslot);
				   m_stats.blindi4to8trans++;
				   free_node(oldblindileaf);
				   return leaf_new; 
			   }
		    default :
                std::cout << "Error: blindileaftype is " << leaf_old->blindileaftype << std::endl;
                getchar();
	    }
    ...
    /// Transfer from btree_nodes to blini_node 
    blindi_leaf_node1* transfer_btree_node_blindi1_node(node* n, node* parent_n, int parentslot)
    {

	blindi_leaf_node1 *blindileaf = allocate_blindi_leaf1();
	btree_leaf_node *leaf_btree = static_cast<btree_leaf_node*>(n);
        //  copy_all_metadata of the leaf
        blindileaf->level = leaf_btree->level;
        blindileaf->slotuse = leaf_btree->slotuse;
#ifdef BREATHING_SIZE 
        blindileaf->currentmaxslot = leaf_btree->currentmaxslot;
#endif
        blindileaf->prevleaf = leaf_btree->prevleaf;
        blindileaf->nextleaf = leaf_btree->nextleaf;
	...
	free_node(leaf_btree);
	return blindileaf;
    }

    /// Transfer from btree_nodes to blini_node 
    blindi_leaf_node2* transfer_btree_node_blindi2_node(node* n, node* parent_n, int parentslot)
    {

	blindi_leaf_node2 *blindileaf = allocate_blindi_leaf2();
	btree_leaf_node *leaf_btree = static_cast<btree_leaf_node*>(n);
        //  copy_all_metadata of the leaf
        blindileaf->level = leaf_btree->level;
        blindileaf->slotuse = leaf_btree->slotuse;
    ...
    void connect_newleaf2tree(node *parent_n, int parentslot, leaf_node *old_leaf, leaf_node *new_leaf) 
    {
	    // connect the tree to the new node
	    inner_node *inner = static_cast<inner_node*>(parent_n);
	    inner->childid[parentslot] = new_leaf;
	    // cange the parent pointer/ change the left pointer and the right pointer
	    if(old_leaf->prevleaf) 
	    { 	
		    old_leaf->prevleaf->nextleaf = new_leaf;
	    }	
	    if(old_leaf->nextleaf)
	    { 
		    old_leaf->nextleaf->prevleaf = new_leaf;
	    }
    }

/// Transfer from btree_nodes to blini_node 
    void connect_blindi2x(leaf_node *oldleaf, leaf_node *newleaf, node* parent_n, int parentslot)
    {
        //  copy_all_metadata of the leaf
        newleaf->level = oldleaf->level;
        newleaf->slotuse = oldleaf->slotuse;
        newleaf->prevleaf = oldleaf->prevleaf;
        newleaf->nextleaf = oldleaf->nextleaf;
        newleaf->blindileaftype = oldleaf->blindileaftype * 2; // btree_node
	// connect the tree to the new node
	if (oldleaf != parent_n) // if we are not transfering the root
	{
		inner_node *inner = static_cast<inner_node*>(parent_n);
		inner->childid[parentslot] = newleaf;
		// cange the parent pointer/ change the left pointer and the right pointer
		if(oldleaf->prevleaf) 
		{ 	
			oldleaf->prevleaf->nextleaf = newleaf;
		}	
		if(oldleaf->nextleaf)
		{ 
			oldleaf->nextleaf->prevleaf = newleaf;
		}
	}
    }
 
    /// Split up a btree leaf node into two equally-filled sibling leaves. Returns
    /// the new nodes and it's insertion key in the two parameters.
    void split_btree_leaf_node(btree_leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
        BTREE_ASSERT(leaf->isfull());

        unsigned int mid = (leaf->slotuse >> 1);

        BTREE_PRINT("blindi_btree_hybrid_nodes::split_btree_leaf_node on " << leaf);
#ifdef BREATHING_SIZE_BTREE
	btree_leaf_node *newleaf;
        if (minleafslots + BREATHING_SIZE > leafslotmax) {
		m_stats.leavesslotsbreath += leafslotmax;
        	newleaf = allocate_btree_leaf(leafslotmax);
       }
	else 
       {
		m_stats.leavesslotsbreath += (minleafslots + BREATHING_SIZE);
        	newleaf = allocate_btree_leaf(minleafslots + BREATHING_SIZE);
	}	
#else
        btree_leaf_node *newleaf = allocate_btree_leaf(leafslotmax);
#endif
	...
	if (minleafslots + BREATHING_SIZE <= leafslotmax) 
	{
		m_stats.leavesslotsbreath -= leafslotmax - (minleafslots + BREATHING_SIZE);
		leaf->currentmaxslot = minleafslots + BREATHING_SIZE;
    ...
    /// Transfer from btree_nodes to blini_node 
    void transfer_blindi_node_btree_node(blindi_leaf_node1 *blindileaf, btree_leaf_node *btreeleaf)
    {
        //  copy_all_metadata of the leaf
        btreeleaf->level = blindileaf->level;
        btreeleaf->slotuse = blindileaf->slotuse;
        btreeleaf->prevleaf = blindileaf->prevleaf;
        btreeleaf->nextleaf = blindileaf->nextleaf;
        btreeleaf->blindileaftype = 0; // btree_node

        // transfer the keys/data
        for (int i=0; i < blindileaf->slotuse; i++) {  
		btreeleaf->slotdata[i] = *(data_type*)get_key_blindi(blindileaf, i);
		btreeleaf->slotkey[i] = *(_Key*)get_key_blindi(blindileaf, i);
	}
    }
     /// Split up a blindi leaf node into two equally-filled sibling leaves. Returns
    /// the new nodes and it's insertion key in the two parameters.
    void split_blindi_leaf_node1(blindi_leaf_node1* leaf, key_type* _newkey, node** _newleaf)
    {
        unsigned int mid = (leaf->slotuse >> 1);
        blindi_leaf_node1 *newleaf = allocate_blindi_leaf1();
        newleaf->slotuse = leaf->slotuse - mid;
        newleaf->nextleaf = leaf->nextleaf;
        if (newleaf->nextleaf == NULL) {
            BTREE_ASSERT(leaf == m_tailleaf);
...
//        *_newkey = leaf->blindi_hybrid_node.get_mid_key_in_node();
        ...
        leaf->nextleaf = newleaf;
        newleaf->prevleaf = leaf;
        *_newkey = leaf->blindi_hybrid_node.get_max_key_in_node();
        *_newleaf = newleaf;
    }
    void split_blindi_leaf_node2(blindi_leaf_node2* leaf, key_type* _newkey, node** _newleaf)
    {
        unsigned int mid = (leaf->slotuse >> 1);
        blindi_leaf_node2 *newleaf = allocate_blindi_leaf2();
        newleaf->slotuse = leaf->slotuse - mid;
        newleaf->nextleaf = leaf->nextleaf;
        if (newleaf->nextleaf == NULL) {
            BTREE_ASSERT(leaf == m_tailleaf);
...
//        *_newkey = leaf->blindi_hybrid_node.get_mid_key_in_node();
    ...
    void split_blindi_leaf_node4(blindi_leaf_node4* leaf, key_type* _newkey, node** _newleaf)
    {
        unsigned int mid = (leaf->slotuse >> 1);
        blindi_leaf_node4 *newleaf = allocate_blindi_leaf4();
        newleaf->slotuse = leaf->slotuse - mid;
        newleaf->nextleaf = leaf->nextleaf;
        if (newleaf->nextleaf == NULL) {
            BTREE_ASSERT(leaf == m_tailleaf);
...
//        *_newkey = leaf->blindi_hybrid_node.get_mid_key_in_node();
    ...
    void split_blindi_leaf_node8(blindi_leaf_node8* leaf, key_type* _newkey, node** _newleaf)
    {
        unsigned int mid = (leaf->slotuse >> 1);
        blindi_leaf_node8 *newleaf = allocate_blindi_leaf8();
        newleaf->slotuse = leaf->slotuse - mid;
        newleaf->nextleaf = leaf->nextleaf;
        if (newleaf->nextleaf == NULL) {
            BTREE_ASSERT(leaf == m_tailleaf);
...
//        *_newkey = leaf->blindi_hybrid_node.get_mid_key_in_node();
    ...
    void split_inner_node(inner_node* inner, key_type* _newkey, node** _newinner, unsigned int addslot)
    {
        ...
        BTREE_PRINT("blindi_btree_hybrid_nodes::split_inner: mid " << mid << " addslot " << addslot);
        ...
        BTREE_PRINT("blindi_btree_hybrid_nodes::split_inner_node on " << inner << " into two nodes " << mid << " and " << inner->slotuse - (mid + 1) << " sized");
...
public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend)
    {
        ...
        BTREE_PRINT("blindi_btree_hybrid_nodes::bulk_load, level 0: " << m_stats.itemcount << " items into " << num_leaves << " leaves with up to " << ((iend - ibegin + num_leaves-1) / num_leaves) << " items per leaf.");

        Iterator it = ibegin;
        for (size_t i = 0; i < num_leaves; ++i)
        {
            // allocate new leaf node
            leaf_node* leaf = allocate_btree_leaf(leafslotmax);
        ...
        // if the blindi_btree_hybrid_nodes is so small to fit into one leaf, then we're done.
        ...
        BTREE_PRINT("blindi_btree_hybrid_nodes::bulk_load, level 1: " << num_leaves << " leaves in " << num_parents << " inner nodes with up to " << ((num_leaves + num_parents-1) / num_parents) << " leaves per inner node.");
        ...
        for (int level = 2; num_parents != 1; ++level)
        {
            ...
            BTREE_PRINT("blindi_btree_hybrid_nodes::bulk_load, level " << level << ": " << num_children << " children in " << num_parents << " inner nodes with up to " << ((num_children + num_parents-1) / num_parents) << " children per inner node.");
...
private:
    ...
    struct result_t
    {
        ...
        inline result_t(result_flags_t f = blindi_btree_hybrid_nodes_ok)
            : flags(f), lastkey()
        { }
        ...
        inline result_t& operator|= (const result_t &other)
        {
            ...
            if (other.has(blindi_btree_hybrid_nodes_update_lastkey))
...
public:
    ...
    {
        BTREE_PRINT("blindi_btree_hybrid_nodes::erase_one(" << key << ") on btree size " << size());
        ...
        if (!result.has(blindi_btree_hybrid_nodes_not_found))
            --m_stats.itemcount;
        ...
        return !result.has(blindi_btree_hybrid_nodes_not_found);
    }
    ...
    {
        BTREE_PRINT("blindi_btree_hybrid_nodes::erase_iter(" << iter.currnode << "," << iter.currslot << ") on btree size " << size());
...
private:
                               ...
                               inner_node *parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            if (slot >= leaf->slotuse || !key_equal(key, leaf->slotkey[slot]))
            {
                ...
                return blindi_btree_hybrid_nodes_not_found;
            }
            ...
            result_t myres = blindi_btree_hybrid_nodes_ok;
            ...
            if (slot == leaf->slotuse)
            {
                ...
                else
                {
                    if (leaf->slotuse >= 1)
                    {
                        ...
                        myres |= result_t(blindi_btree_hybrid_nodes_update_lastkey, leaf->slotkey[leaf->slotuse - 1]);
                    }
                    else
                    {
            ...
            if (leaf->isunderflow() && !(leaf == m_root && leaf->slotuse >= 1))
            {
                ...
                if (leftleaf == NULL && rightleaf == NULL)
                {
                    ...
                    return blindi_btree_hybrid_nodes_ok;
                }
        ...
        else // !curr->isleafnode()
        {
            ...
            if (result.has(blindi_btree_hybrid_nodes_update_lastkey))
            {
                if (parent && parentslot < parent->slotuse)
                {
                ...
                else
                {
                    ...
                    myres |= result_t(blindi_btree_hybrid_nodes_update_lastkey, result.lastkey);
                }
            }

            if (result.has(blindi_btree_hybrid_nodes_fixmerge))
            {
     ...
     * first search for that leaf node in the sublindi_btree_hybrid_nodes containing all pairs of
     * the same key. This sublindi_btree_hybrid_nodes can be very large, even the whole tree,
    ...
    result_t merge_leaves(leaf_node* left, leaf_node* right, inner_node* parent)
    {
        ...
        return blindi_btree_hybrid_nodes_fixmerge;
    }
    ...
    static result_t shift_left_leaf(leaf_node *left, leaf_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        if (parentslot < parent->slotuse) {
            ...
            return result_t(blindi_btree_hybrid_nodes_update_lastkey, left->slotkey[left->slotuse - 1]);
        }
    }
...
private:
    static void print_key(std::ostream &os, uint8_t* key) {
	os << " ( ";
    ...
    static void print_node(std::ostream &os, node* node, unsigned int depth=0, bool recursive=false) 
    {
	    ...
	    if(!(leafnode->isblindileafnode()))
	    {	 	
		    os << "btree_leaf_node " << std::endl;
		    btree_leaf_node *btreeleafnode = static_cast<btree_leaf_node*>(node);
		    for(unsigned int i = 0; i < depth; i++) os << "  ";
		    os << "  leaf prev " << btreeleafnode->prevleaf << " next " << btreeleafnode->nextleaf << std::endl;

		    for(unsigned int i = 0; i < depth; i++) os << "  ";

		    for (unsigned int slot = 0; slot < btreeleafnode->slotuse; ++slot)
		    {
			    print_key (os,  (uint8_t*) &btreeleafnode->slotkey[slot]);
		    }
		    os << std::endl;
	    }
	    else 
	    {
		    os << "blindi_leaf_node("<<(int) leafnode->blindileaftype <<") " <<  std::endl;
		     for(unsigned int i = 0; i < depth; i++) os << "  ";
		     ...
		     for (unsigned int slot = 0; slot < leafnode->slotuse; ++slot)
		     {
			     print_key (os, get_key_blindi_static(leafnode, slot));
		     }
		     os << std::endl;
	    }
        }
...
public:
    ...
    void verify() const
    {
        ...
        if (m_root)
        {
            ...
            if ( vstats.itemcount !=  m_stats.itemcount || vstats.leaves !=  m_stats.leaves) {
		 std::cout << "vstats.itemcount == m_stats.itemcount "<< vstats.itemcount << " " <<  m_stats.itemcount  << " vstats.leaves == m_stats.leaves "<<  vstats.leaves <<" " << m_stats.leaves << "  vstats.innernodes == m_stats.innernodes " <<   vstats.innernodes<< " " <<  m_stats.innernodes << std::endl;
		 exit(0);
	    }
            verify_leaflinks();
        }
...
private:
    ...
    void verify_node(node* n, key_type* minkey, key_type* maxkey, tree_stats &vstats) const
    {
        ...
        if (n->isleafnode())
        {
            const leaf_node *leaf_tmp = static_cast<const leaf_node*>(n);
	    if (!leaf_tmp->isblindileafnode()) 
	    {	
		    const btree_leaf_node *leaf = static_cast<const btree_leaf_node*>(leaf_tmp);
		    for(unsigned short slot = 0; slot < leaf->slotuse - 1; ++slot)
		    {
			    if(!key_lessequal(leaf->slotkey[slot], leaf->slotkey[slot + 1]))  {
			         std::cout << " ERROR: key_lessequal(leaf->slotkey[slot], leaf->slotkey[slot + 1] slot  " <<  slot << " leaf->slotuse "<< leaf->slotuse  << std::endl;
				    exit(0);
			    }
		    }
	    ...
	    { 		 
		    ...
		    leaf_node *leaf = static_cast<leaf_node*>(n);
		    //            if( leaf != m_root && leaf->isunderflow() ) { 
		    //                std::cout << "ERROR:  leaf != m_root && leaf->isunderflow() " << std::endl;
		    //		exit(0);
		    //	    }
		    if( leaf != m_root && leaf->slotuse == 0 ){
			    std::cout << " ERROR: leaf->slotuse == 0 " << std::endl;
			    exit(0);
		    }	
                    if (leaf->slotuse > 0) {  
			    for(unsigned short slot = 0; slot < leaf->slotuse - 1; ++slot)
			    {	
				    key_type _small_key = key_type();
				    key_type _large_key = key_type();
				    _small_key = *((_Key*)get_key_blindi_static(leaf, slot));
				    _large_key =  *((_Key*)get_key_blindi_static(leaf, slot + 1));
				    if (key_greater(_small_key, _large_key)) 
				    {
					    print_node(std::cout, m_root, 0, true);
					    std::cout << "key_greater(_small_key, _large_key) " << " slot "  << slot<< std::endl;
					    exit(0);
				    }
			    }
		    }
// TBD              *minkey = *((_Key*)get_key_blindi_static(leaf, 0));
//		    *maxkey = *((_Key*)get_key_blindi_static(leaf, ));		

//		    		    if (leaf->nextleaf != NULL) { 	 
//		    			    key_type _max_key = key_type();
//		    			    key_type _next_key = key_type();
//		    			    _max_key = leaf->blindi_hybrid_node.get_max_key_in_node();
//		    			    _next_key =  leaf->nextleaf->blindi_hybrid_node.get_min_key_in_node();
//		    			    if (key_greater(_max_key, _next_key)) {
//		    				    std::cout << "key_greater(_max_key, _next_key)) " << std::endl;
//		    				    exit(0);
	...
	else // !n->isleafnode()
	{
		...
		if( inner != m_root && inner->isunderflow() ) {
			std::cout << " ERROR: inner != m_root && inner->isunderflow() " << std::endl;
			exit(0);
		}
		if ( inner->slotuse == 0 ) {
			std::cout << "ERROR: inner->slotuse == 0 " << std::endl;
			exit(0);
		}	

		for(unsigned short slot = 0; slot < inner->slotuse - 1; ++slot)
		{
			if(key_greater(inner->slotkey[slot], inner->slotkey[slot+1]))
			{
				print_node(std::cout, m_root, 0, true);
				std::cout << "inner key_greater(inner->slotkey[slot+1], inner->slotkey[slot] " << std::endl;
				exit(0);
			}
		}
		...
		for(unsigned short slot = 0; slot <= inner->slotuse; ++slot)
		{
			node *subnode = inner->childid[slot];
			key_type subminkey = key_type();
			key_type submaxkey = key_type();
			if(subnode->level + 1 != inner->level) 
			{
				std::cout << "subnode->level + 1 != inner->level" << std::endl;
				exit(0);
			}
...
/* TBD
			if (slot == 0)
			{
				*minkey = subminkey;
				...
				if(!(key_greaterequal(subminkey, inner->slotkey[slot-1]))) {
					std::cout << " !(key_greaterequal(subminkey, inner->slotkey[slot-1])) inner  "<< inner  <<" slot " << (int)slot << std::endl;
					exit(0);
				}
			...
			if (slot == inner->slotuse)
				*maxkey = submaxkey;
				...
				if (!(key_equal(inner->slotkey[slot], submaxkey))) {
					std::cout << "!(key_equal(inner->slotkey[slot], submaxkey)) " << subnode << std::endl;
					exit(0);
				}
			}
*/
			if (inner->level == 1 && slot < inner->slotuse)
			{
				...
				if (!((leafa->nextleaf == leafb) && (leafa == leafb->prevleaf))) {
					print_node(std::cout, m_root, 0, true);
					std::cout << "!((leafa->nextleaf == leafb) && (leafa == leafb->prevleaf))) " << std::endl;
			...
			if (inner->level == 2 && slot < inner->slotuse)
			{
				...
				if ( !((leafa->nextleaf == leafb) &&  (leafa == leafb->prevleaf))) {
					print_node(std::cout, m_root, 0, true);
					std::cout << "( !((leafa->nextleaf == leafb) &&  (leafa == leafb->prevleaf)))" << "  leafa " << leafa << " leafb " << leafb << " leafa->nextleaf: " << leafa->nextleaf << " leafb->prevleaf: " << leafb->prevleaf << std::endl; 	
                                        exit(0);  	
				}		
				(void)leafa; (void)leafb;
			}
...
private:
    ...
    struct dump_header
    {
        /// "stx-blindi_btree_hybrid_nodes", just to stop the restore() function from loading garbage
        char            signature[12];
        ...
        inline void fill()
        {
            ...
            version = 0;
            key_type_size = sizeof(typename blindi_btree_hybrid_nodes_self::key_type);
            data_type_size = sizeof(typename blindi_btree_hybrid_nodes_self::data_type);
            leafslots = blindi_btree_hybrid_nodes_self::leafslotmax;
            innerslots = blindi_btree_hybrid_nodes_self::innerslotmax;
            allow_duplicates = blindi_btree_hybrid_nodes_self::allow_duplicates;
        }
...
public:
    ...
    bool restore(std::istream &is)
    {
        ...
        {
            BTREE_PRINT("blindi_btree_hybrid_nodes::restore: file header does not match instantiation signature.");
            return false;
        }
