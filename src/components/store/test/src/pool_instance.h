...
private:
	...
	)
	{
		...
		return kvstore_->create_pool(n, sz_);
	}
public:
	pool_instance(std::shared_ptr<IKVStore> kvstore_, common::string_view pool_name_, std::size_t sz_ = MB(32))
		: pool_named(kvstore_, pool_name_)
		, pool_opened(kvstore_, create_pool(kvstore_, _pool_name, sz_))
	{}
};
