...
namespace
{
	/* Helper for gsl::span, which cannot construct a span from a vector. std::span can */
	template <typename R>
		auto range_to_span(const R && r)
		{
			return gsl::span<const typename R::value_type>(&*std::begin(r), r.size());
		}
}
...
{
	return std::make_unique<nupm::dax_manager>(ls_, range_to_span(parse_devdax_string(ls_.debug_level(), dax_map)), force_reset, dax_span);
}
