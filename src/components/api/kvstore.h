...
public:
   ...
   */
  virtual pool_t create_pool(const string_view pool_name,
                             const size_t       size,
                             flags_t            flags              = 0,
   ...
   * unsupported
   */
  virtual pool_t open_pool(const string_view pool_name,
                           flags_t flags = 0,
  ...
  template <typename K>
    status_t put_direct(const pool_t       pool,
      K key,
                              ...
                              flags_t            flags  = FLAGS_NONE)
    {
      return put_direct(pool, to_key(key), values, handles, flags);
    }
 ...
 /**
  ...
  virtual status_t set_attribute(const pool_t                 pool,
                                 ...
                                 const string_view_key        key = string_view_key{})
  {
  ...
  */
   ...
   */
  virtual status_t ioctl(string_view_byte command)
  {
    return error_value(E_NOT_SUPPORTED, command);
  }
